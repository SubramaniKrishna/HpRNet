...
dir_files = './paramsconcat/'
list_pth_files = glob.glob(dir_files + '*.pth')

# Display the available parameter files
print('Available state files to load are(for Concatenative cVAE) : ')
...
# print(list_params)
file_load_concat = list_pth_files[idx]
...
cVAE_C.load_state_dict(torch.load(file_load_concat,map_location = 'cpu'))
...
dir_save = './net_recon/ConcatNet_' + list_pth_files[idx].split('_')[0][-7:-1] + '/'
try: 
	os.mkdir(dir_save) 
...
for k in data.keys():
	...
	if(octave in list_octave and note in list_notes and loudness in list_loudness and style in list_style):
		...
		inpNet = torch.cat((inpH,inpR), dim = 1)

		if(flag_cond == True):
			xC_recon, muC, sigma_covarC, zC = cVAE_C.forward(inpNet.float(),(p.float()/pnf).view(-1,1))
		else:
			xC_recon, muC, sigma_covarC, zC = cVAE_C.forward(inpNet.float())
			
		ccH_recon_cVAE = xC_recon[:,:cc_keep_H]
		ccR_recon_cVAE = xC_recon[:,cc_keep_H:cc_keep_H + cc_keep_R]
		# print(inpH.shape,ccH_recon_cVAE.shape,inpR.shape,ccR_recon_cVAE.shape)
